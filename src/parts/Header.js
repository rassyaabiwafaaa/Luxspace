...
export default function Header({ theme, position }) {
  ...
  const [isCartChange, setIsChartChange] = useState(false);

  const prevCart = useRef(state?.cart || {});

  useLayoutEffect(() => {
    if (prevCart.current !== prevCart) {
      prevCart.current = state.cart || {};
      setIsChartChange(true);
      setTimeout(() => {
        setIsChartChange(false);
      }, 550);
    }
  }, [state.cart]);
          ...
          <div className="w-auto">
            ...
            >
              <li className="mx-3 py-6 md:py-0">
                <Link to="/showcase" className={["hover:underline", theme === "white" ? "text-black md:text-black" : "text-black md:text-black"].join()}>
                  Showcase
                </Link>
              </li>
              <li className="mx-3 py-6 md:py-0">
                <Link to="/catalog" className={["hover:underline", theme === "white" ? "text-black md:text-black" : "text-black md:text-black"].join()}>
                  Catalog
                </Link>
              </li>
              <li className="mx-3 py-6 md:py-0">
                <Link to="/delivery" className={["hover:underline", theme === "white" ? "text-black md:text-black" : "text-black md:text-black"].join()}>
                  Delivery
                </Link>
              </li>
              <li className="mx-3 py-6 md:py-0">
                <Link to="/rewards" className={["hover:underline", theme === "white" ? "text-black md:text-black" : "text-black md:text-black"].join()}>
                  Rewards
                </Link>
              </li>
            </ul>
              ...
              <li className="ml-6">
                <Link
                  ...
                  className={[
                    ...
                    theme === "white" ? "text-black md:text-black" : "text-black md:text-black",
                    state.cart && Object.keys(state.cart > 0 ? "cart-field" : "", isCartChange ? "animate-bounce" : ""),
                  ].join(" ")}
                >
